...
export const RecipeCard = ({ recipe, onToggleFavorite, isFavorite }: RecipeCardProps) => {
  const [showFullRecipe, setShowFullRecipe] = useState(false);
  
  const totalTime = (recipe.prep_time_minutes || 0) + (recipe.cook_time_minutes || 0);
  const discountedIngredients = recipe.ingredients.filter(ing => ing.isDiscounted);
      ...
      <CardHeader className="pb-3">
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {recipe.difficulty_level && (
            <Badge className={difficultyColors[recipe.difficulty_level]}>
        ...
        {discountedIngredients.length > 0 && (
          <div className="mt-2">
            <Badge className="bg-savings text-white">
              {discountedIngredients.length} discounted ingredients
            </Badge>
          </div>
        )}
      </CardHeader>
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="space-y-1">
              {recipe.ingredients.slice(0, showFullRecipe ? undefined : 3).map((ingredient, index) => (
                <div 
                  key={index} 
                  className={`text-sm flex items-center gap-2 ${
                    ingredient.isDiscounted ? 'text-savings font-medium' : 'text-muted-foreground'
                  }`}
                >
                  <span className="w-2 h-2 rounded-full bg-current opacity-50" />
                  {ingredient.amount} {ingredient.unit} {ingredient.name}
                  {ingredient.isDiscounted && (
                    <Badge variant="outline" className="text-xs bg-savings/10 text-savings border-savings">
                      ON SALE
                    </Badge>
                  )}
                </div>
              ))}
              {!showFullRecipe && recipe.ingredients.length > 3 && (
