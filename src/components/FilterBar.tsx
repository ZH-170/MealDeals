...
export const FilterBar = ({ filters, onFiltersChange, categories }: FilterBarProps) => {
  ...
  const activeFiltersCount = filters.stores.length + filters.categories.length + 
    (filters.searchTerm ? 1 : 0) + (filters.minDiscount > 0 ? 1 : 0) + 
          ...
          <CollapsibleContent className="mt-4">
            <Card>
              <CardContent className="p-4 space-y-4">
                {/* Stores Filter */}
                <div>
                  <h4 className="text-sm font-semibold mb-2">Stores</h4>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold">Min Discount %</label>
                    <Input
                      type="number"
                      value={filters.minDiscount}
                      onChange={(e) => updateFilters({ minDiscount: Number(e.target.value) })}
                      min="0"
                      max="100"
                      className="mt-1"
                    />
